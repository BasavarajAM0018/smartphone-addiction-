...
import os
import datetime
import ast   # <--- IMPORTANT: needed to parse stored inputs
...
def init_db():
    ...
    # logs table includes category and age columns (age stored as TEXT)
    c.execute("""
...
# ---------------------------------------------------------
#   HELPER: STAGE DETAILS
# ---------------------------------------------------------
def get_stage_details(addiction_prob):
    if addiction_prob >= 75:
...
def predict():
    ...
    if request.method == "POST":
        ...
        yes_count = sum(inputs)

        # MODEL OR MANUAL %
        try:
            if model and hasattr(model, "predict_proba"):
                p = model.predict_proba([inputs])[0]
                addiction_prob = round(p[1] * 100, 2)
            else:
                addiction_prob = round(yes_count / 18 * 100, 2)
        except Exception:
            addiction_prob = round(yes_count / 18 * 100, 2)

        # FORCE EXTREMES
        if yes_count == 18:
            addiction_prob = 100.0
        if yes_count == 0:
            addiction_prob = 0.0
        ...
        # Get stage/category, symptoms and tips using helper
        category, symptoms, tips = get_stage_details(addiction_prob)

        # SAVE LOG (store age as well)
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute(
            "INSERT INTO logs (user_id, inputs, prediction, category, age, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
            (
                session["user_id"],
                str(inputs),
...
def logs():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, inputs, prediction, category, age, timestamp FROM logs WHERE user_id=? ORDER BY id DESC",
        (session["user_id"],)
    )
    rows = c.fetchall()
    conn.close()
    ...
    logs_data = []
    for r in rows:
        log_id, inputs_str, prediction, category, age, timestamp = r
